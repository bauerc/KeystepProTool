---
name: spec-guardian
description: Checks proposed or existing changes to KSPKit format logic against analysis/KeyStepPro_Format_Spec.md's known traps (index spaces, existence vs audibility, note placement, drum lane mapping, gate ladder). Use before or after editing format code, instead of reading the full spec in the main conversation. Do not use for changes confined to KSPSwiftCLI, KSPRun or KSPApp, or for non-format changes.
...
- **Placing a melodic note is 8 keys**: `50`, `109`-`113` by note ordinal, plus `48` by step in
  slot 1, plus `40`. `49` is never written. `Mutate.placeNote` should be the only code
...
- **Gate is a measured index** (`stored = detent - 1`, 128 rungs) — flag any code computing gate
  values without going through `Constants.gateTable`.
...
- **Time shift and swing are measured** (M7) — shift is `stored = 49 + displayed` over stored 0–99,
  ...
  `Constants.timeShiftTicks`. Swing is an absolute 50–75 % per pattern, delaying even steps, and
...
- **Architecture boundary**: `KSPKit` takes no dependency, prints nothing, reads no
  `CommandLine.arguments` and decides no file paths. Flag any violation. (Not a spec question — no file to read.)

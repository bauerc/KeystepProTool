...
def test_a_track_past_the_devices_limit_is_split_and_chained(
    m6_song: Path, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """Source track 6 runs past 64 steps and says so in the split's words."""
    destination = tmp_path / "out.KeyStepPro"
    argv = [str(m6_song), "-o", str(destination)]

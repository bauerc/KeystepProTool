...
//
//   swift tools/gen_bulk_plan.swift
//
// Re-run only on a firmware update, then run the tests: BulkFastTests holds the plan to
// fixtures/bulk_fast_requests.txt. The vendor file carries trailing commas before closing
// brackets, the project files' dialect, and duplicate `desc` and `comment` keys, which are harmless.

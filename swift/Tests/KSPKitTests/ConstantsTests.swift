...
/// Known independently of the transcription: six from the sample projects, one from a capture.
private let crossCheckPoints = [
    (7, 0.5), (11, 1.0), (19, 2.0), (27, 3.0), (29, 3.5), (31, 4.0), (36, 5.25),
]

/// The file holds the device's 2-decimal rendering, so comparisons round half to even: 0.625 -> 0.62.
@Suite struct GateLadderTests {
    ...
    @Test func everyTranscribedDetentMatchesTheTable() {
        // Stored value = detent index - 1, for all 128 rungs.
...
@Suite struct EncodingTests {
    ...
    /// Rounding to the nearest rung would produce a file that loads cleanly and plays wrong.
...
/// One row of the pattern-bits sweep: a stored value and what the device displayed for it.
...
/// Parameter 52 is flattened [lane][part], lane-major, 7 bits per entry, 10 entries per lane.
